...
import { HandlaggningsstatusPageComponent } from './handlaggningsstatus-page.component';
...
describe('HandlaggningsstatusPageComponent', () => {
  let component: HandlaggningsstatusPageComponent;
  let fixture: ComponentFixture<HandlaggningsstatusPageComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [HandlaggningsstatusPageComponent],
      providers: [
      ...
      .overrideTemplate(HandlaggningsstatusPageComponent, '<div></div>')
      .compileComponents();

    fixture = TestBed.createComponent(HandlaggningsstatusPageComponent);
  ...
  it('sets document and clears isLoading when route emits and audit resolves', () => {
    const doc = makeNuxeoDocument({ uid: 'uid-1', properties: { 'handlaggningsstatus:kod': 'A1' } });
  ...
  it('detailItems returns items from document properties with native schema', () => {
    const doc = makeNuxeoDocument({
      uid: 'uid-1',
      properties: { 'handlaggningsstatus:kod': 'HS1', 'dc:title': 'Test Title' },
    });
    routeParams$.next({ id: 'uid-1' });
    auditSubject$.next(doc);
    ...
    expect(kodItem?.value).toBe('HS1');
  });

  it('detailItems falls back to cv: props when native schema is absent', () => {
    const doc = makeNuxeoDocument({
      uid: 'uid-1',
  ...
  it('handleEditResult calls editDocument with native schema key when native prop is defined', () => {
    const doc = makeNuxeoDocument({
      uid: 'uid-1',
      properties: { 'handlaggningsstatus:kod': 'HS1' },
    });
    ...
    component.handleEditResult({ code: 'HS2' });
    expect(nuxeoApiSpy.editDocument).toHaveBeenCalledWith(
      'uid-1',
      jasmine.objectContaining({ 'handlaggningsstatus:kod': 'HS2' })
    );
  });

  it('handleEditResult uses cv: fallback when native schema prop is undefined', () => {
    const doc = makeNuxeoDocument({
      uid: 'uid-1',
      properties: { 'cv:kod': 'CV1' } as Record<string, unknown>,
    });
  ...
  it('saveVocabDocument shows success notification after successful edit', () => {
    const doc = makeNuxeoDocument({ uid: 'uid-1', properties: { 'handlaggningsstatus:kod': 'HS1' } });
    routeParams$.next({ id: 'uid-1' });
    auditSubject$.next(doc);
