...
import { BeredningsbeslutPageComponent } from './beredningsbeslut-page.component';
...
describe('BeredningsbeslutPageComponent', () => {
  let component: BeredningsbeslutPageComponent;
  let fixture: ComponentFixture<BeredningsbeslutPageComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [BeredningsbeslutPageComponent],
      providers: [
      ...
      .overrideTemplate(BeredningsbeslutPageComponent, '<div></div>')
      .compileComponents();

    fixture = TestBed.createComponent(BeredningsbeslutPageComponent);
  ...
  it('sets document and clears isLoading when route emits and audit resolves', () => {
    const doc = makeNuxeoDocument({ uid: 'uid-1', properties: { 'beredningsbeslut:kod': 'BB1' } });
  ...
  it('detailItems returns items from document properties with cv: schema', () => {
    const doc = makeNuxeoDocument({
      uid: 'uid-1',
      properties: { 'cv:kod': 'CV1', 'dc:title': 'Beredningsbeslut Title' },
    });
    routeParams$.next({ id: 'uid-1' });
    auditSubject$.next(doc);
  ...
  it('detailItems falls back to beredningsbeslut: props when cv: schema is absent', () => {
    const doc = makeNuxeoDocument({
      uid: 'uid-1',
      properties: { 'beredningsbeslut:kod': 'BB1' } as Record<string, unknown>,
    });
    routeParams$.next({ id: 'uid-1' });
    auditSubject$.next(doc);
    const items = component.detailItems();
    const kodItem = items.find(i => i.label === 'Kod');
    expect(kodItem?.value).toBe('BB1');
  });
  ...
  it('handleEditResult uses beredningsbeslut: fallback when cv: prop is undefined', () => {
    const doc = makeNuxeoDocument({
      uid: 'uid-1',
      properties: { 'beredningsbeslut:kod': 'BB1' } as Record<string, unknown>,
    });
    ...
    component.handleEditResult({ code: 'BB2' });
    expect(nuxeoApiSpy.editDocument).toHaveBeenCalledWith(
      'uid-1',
      jasmine.objectContaining({ 'beredningsbeslut:kod': 'BB2' })
    );
  });
