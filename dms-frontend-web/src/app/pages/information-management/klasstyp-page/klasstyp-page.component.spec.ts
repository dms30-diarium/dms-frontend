...
import { KlasstypPageComponent } from './klasstyp-page.component';
...
describe('KlasstypPageComponent', () => {
  let component: KlasstypPageComponent;
  let fixture: ComponentFixture<KlasstypPageComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideTemplate(KlasstypPageComponent, '<div></div>')
      .compileComponents();

    fixture = TestBed.createComponent(KlasstypPageComponent);
  ...
  it('sets document and clears isLoading when route emits and audit resolves', () => {
    const doc = makeNuxeoDocument({ uid: 'uid-1', properties: { 'klasstyp:kod': 'K1' } });
  ...
  it('detailItems returns items from document properties with cv: schema', () => {
    const doc = makeNuxeoDocument({
      uid: 'uid-1',
      properties: { 'cv:kod': 'CV1', 'dc:title': 'Klasstyp Title' },
    });
    routeParams$.next({ id: 'uid-1' });
    auditSubject$.next(doc);
  ...
  it('detailItems falls back to klasstyp: props when cv: schema is absent', () => {
    const doc = makeNuxeoDocument({
      uid: 'uid-1',
      properties: { 'klasstyp:kod': 'K1' } as Record<string, unknown>,
    });
    routeParams$.next({ id: 'uid-1' });
    auditSubject$.next(doc);
    const items = component.detailItems();
    const kodItem = items.find(i => i.label === 'Kod');
    expect(kodItem?.value).toBe('K1');
  });
  ...
  it('handleEditResult uses klasstyp: fallback when cv: prop is undefined', () => {
    const doc = makeNuxeoDocument({
      uid: 'uid-1',
      properties: { 'klasstyp:kod': 'K1' } as Record<string, unknown>,
    });
    ...
    component.handleEditResult({ code: 'K2' });
    expect(nuxeoApiSpy.editDocument).toHaveBeenCalledWith('uid-1', jasmine.objectContaining({ 'klasstyp:kod': 'K2' }));
  });
