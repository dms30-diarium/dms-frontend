...
import { BeslutstyperPageComponent } from './beslutstyper-page.component';
...
describe('BeslutstyperPageComponent', () => {
  let component: BeslutstyperPageComponent;
  let fixture: ComponentFixture<BeslutstyperPageComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [BeslutstyperPageComponent],
      providers: [
      ...
      .overrideTemplate(BeslutstyperPageComponent, '<div></div>')
      .compileComponents();

    fixture = TestBed.createComponent(BeslutstyperPageComponent);
  ...
  it('sets document and clears isLoading when route emits and audit resolves', () => {
    const doc = makeNuxeoDocument({ uid: 'uid-1', properties: { 'beslutstyper:kod': 'B1' } });
  ...
  it('detailItems returns items from document properties with native schema', () => {
    const doc = makeNuxeoDocument({
      uid: 'uid-1',
      properties: { 'beslutstyper:kod': 'B1', 'dc:title': 'Beslutstyp Title' },
    });
    routeParams$.next({ id: 'uid-1' });
    auditSubject$.next(doc);
  ...
  it('handleEditResult calls editDocument with native schema key when native prop is defined', () => {
    const doc = makeNuxeoDocument({
      uid: 'uid-1',
      properties: { 'beslutstyper:kod': 'B1' },
    });
    routeParams$.next({ id: 'uid-1' });
    auditSubject$.next(doc);
    ...
    component.handleEditResult({ code: 'B2' });
    expect(nuxeoApiSpy.editDocument).toHaveBeenCalledWith(
      'uid-1',
      jasmine.objectContaining({ 'beslutstyper:kod': 'B2' })
    );
  });
