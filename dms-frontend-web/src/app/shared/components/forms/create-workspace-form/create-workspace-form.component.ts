...
import {
  WORKSPACE_CREATE_ERROR_MESSAGE,
  WORKSPACE_CREATE_SUCCESS_MESSAGE,
...
@Component({
  selector: 'nuxeo-create-workspace-form',
  standalone: true,
...
export class CreateWorkspaceFormComponent implements OnInit {
  ...
  createWorkspaceConfig = signal<FieldConfig[]>([
    {
      type: 'input',
      name: 'title',
  ...
  ngOnInit(): void {
    this.nuxeoApi
      .getEmptyWithDefaults(this.path(), 'Workspace')
      .pipe(tap(result => this.defaultPayload.set(result)))
      .subscribe();

    initDirectoryOptions(this.directoryOptions, (field, opts) =>
      updateFormFieldOptions(this.createWorkspaceConfig, field, opts)
    );
  }
  ...
  createDocument(event: Record<string, unknown>): void {
    ...
    const docTitle = String(event['title'] ?? '').trim() || 'workspace';
      ...
      .pipe(
        tap(result => {
          this.store.notification.set({
            show: true,
            variation: 'success',
            text: WORKSPACE_CREATE_SUCCESS_MESSAGE,
          });
        ...
        catchError(() => {
          this.store.notification.set({
            show: true,
            variation: 'danger',
            text: WORKSPACE_CREATE_ERROR_MESSAGE,
          });
