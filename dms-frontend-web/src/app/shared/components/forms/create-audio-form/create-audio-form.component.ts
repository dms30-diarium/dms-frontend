...
import {
  AUDIO_CREATE_ERROR_MESSAGE,
  AUDIO_CREATE_SUCCESS_MESSAGE,
  AUDIO_UPLOAD_ERROR_MESSAGE,
  AUDIO_UPLOAD_REQUIRED_MESSAGE,
...
@Component({
  selector: 'nuxeo-create-audio-form',
  standalone: true,
  imports: [ReactiveFormsModule, GeneralFormComponent],
  templateUrl: './create-audio-form.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CreateAudioFormComponent implements OnInit {
  ...
  createAudioConfig = signal<FieldConfig[]>([
    {
      type: 'input',
      name: 'title',
  ...
  ngOnInit(): void {
    ...
    initDirectoryOptions(this.directoryOptions, (field, opts) =>
      updateFormFieldOptions(this.createAudioConfig, field, opts)
    );
  }
  ...
  uploadFile(files: UploadedFile[]) {
    if (files.length === 0) {
      ...
      this.createAudioConfig.update(fields =>
      ...
      .subscribe({
        error: () => {
          this.store.notification.set({
            show: true,
            variation: 'danger',
            text: AUDIO_UPLOAD_ERROR_MESSAGE,
          });
        },
      });
  }
  ...
  createDocument(event: Record<string, unknown>): void {
    ...
    if (!this.fileBatchId) {
      this.store.notification.set({
        show: true,
        variation: 'warning',
        text: AUDIO_UPLOAD_REQUIRED_MESSAGE,
      });
      return;
    }

    const titleText = String(event['title'] ?? '').trim();
    const docTitle = titleText || this.uploadedFileName || 'audio';
    ...
    const fullPayload: Partial<NuxeoDocument<Record<string, unknown>>> = {
      ...
      type: this.itemType() || payload.type || 'Audio',
      name: docTitle,
      title: docTitle,
      properties,
    };
      ...
      .pipe(
        ...
        tap(result => {
          this.store.notification.set({
            show: true,
            variation: 'success',
            text: AUDIO_CREATE_SUCCESS_MESSAGE,
          });
        ...
        catchError(() => {
          this.store.notification.set({
            show: true,
            variation: 'danger',
            text: AUDIO_CREATE_ERROR_MESSAGE,
          });
